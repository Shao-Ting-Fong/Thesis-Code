...
## 3. 使用資料庫介紹
### 3.1 資料庫架構簡介
- NBI橋梁資料庫，是隸屬於美國聯邦公路總署(FHWA)之公開的資料庫。
...
- 方案二：投入連續兩次檢測資料訓練模型  
	...
	- 取2018年不需檢測，且近三筆評分未上升之橋梁檢測資料為訓練集。
	訓練集中「前兩筆檢測資料的各項參數」為X_train，「第三筆檢測結果是否為缺陷橋」為y_train，以此訓練模型。
	- 取2018年需要檢測的橋梁為測試集，將每座橋「前兩次檢測的各項參數」投入模型中，使其預測各自「在2018年是否為缺陷橋」。
	測試集兩筆連續資料中同樣不得出現評分上升的情況。  
  
- 方案三：投入連續三次檢測資料訓練模型
	與方案二情況類似：
	- 取2018年不需檢測，且近四筆評分未上升之橋梁檢測資料為訓練集。
	訓練集中「前三筆檢測資料的各項參數」為X_train，「第四筆檢測結果是否為缺陷橋」為y_train，以此訓練模型。
	- 取2018年需要檢測的橋梁為測試集，將每座橋「前三次檢測的各項參數」投入模型中，使其預測各自「在2018年是否為缺陷橋」。
	測試集三筆連續資料中同樣不得出現評分上升的情況。
	
三個方案之訓練集與測試集資料筆數彙整於下表

|       | 訓練集總橋數 | 訓練集缺陷橋數 | 測試集總橋數 | 測試集缺陷橋數 |
| ----- | ---------- | -----------  | ----------| -------------|
| 方案一 | 592,656|293,077|189,795|8,440(4.4%)|
| 方案二 | 445,113|225,570|167,912|7,773 (4.6%)|
| 方案三 | 387,250|198,918|103,904|5,045(4.9%)|

從表中可以發現：
1. 雖然2018年需檢測的橋梁總數不變，但隨著條件越趨嚴格，可納入訓練集與測試集的資料數皆有些許下降。
2. 測試集缺陷橋數僅佔總橋數不到5%，屬不平衡資料(Imbalanced Dataset)之預測問題
3. 訓練集中兩類別資料數不盡相同，本研究使用SMOTE方法增加少數類別之資料筆數，確保模型平衡學習。

### 5.3 預測結果
|       | Recall | Precision | 重大失誤率 | 資源節省率 |
| ----- | ------ | --------  | ---------|  ---------|
| 方案一 | 93.76% |9.11%	     |5.97%	    |     50.68%|
| 方案二 | 94.43% |9.22%      |4.90% 	  |     43.47%|
| 方案三 | 94.55% |10.13%	   |3.11%	    |     27.96%|

隨著投入的連續檢測資料越多，Recall值表現越好，重大失誤率亦有下降，推測主要原因有二：
1. 投入較多連續檢測資料，可供模型更多資訊掌握橋梁資料特性，進而提升其預測表現
2. 隨著對訓練集與測試集的要求越趨嚴格，變化較多、較不規則的橋梁將被剔除，使資料同質性提升，模型也就更容易掌握資料特性。

[單年度模型預測程式碼](code/SingleYearPrediction.ipynb)

### 5.4 重要參數分析
|排名|參數|重要程度|
|---|---|------|
|1	|Superstructure Condition	|14.88%|
|2	|Substructure Condition	|6.68%|
|3	|Age	|6.26%|
|4	|Structural Evaluation	|6.09%|
|5	|Railings	|2.46%|
|6	|Inventory Rating	|2.42%|
|7	|Operating Rating	|2.06%|
|8	|Latitude	|1.86%|
|9	|Longitude	|1.83%|
|10	|Bridge Roadway Width	|1.60%|
|11	|Structure Kind	|1.59%|
|12	|Maximum Span Length	|1.59%|
|13	|Improvement Length	|1.56%|
|14	|Deck Width	|1.53%|
|15	|Average Daily Traffic	|1.52%|
|16	|Structure Length	|1.51%|
|17	|Future Average Daily Traffic	|1.50%|
|18	|Horizontal Clearance	|1.39%|
|19	|Approach Roadway Width	|1.35%|
|20	|Deck Protection	|1.34%|

觀察重要參數前20名，大致可分為五類：
1.	橋梁評估指標或其餘構件評分
2.	橋齡
3.	橋梁設計相關參數（eg.橋種、設計載重、橋梁尺寸...）
4.	地理位置與地區特性(經緯度、交通量)
5.	橋梁維護相關參數
...
[comment]: <> (設定初始檢測年與跳過次數上限-->生成訓練集與測試集資料;)

[comment]: <> (生成訓練集與測試集資料-->訓練預測模型;)

[comment]: <> (訓練預測模型-->A{預測是否為缺陷橋};)

[comment]: <> (A--否-->B{是否可跳過檢測};)

[comment]: <> (B--是-->跳過檢測沿用上次檢測資料)

[comment]: <> (B--否-->仍需實施檢測;)

[comment]: <> (A--是-->仍需實施檢測;)

[comment]: <> (仍需實施檢測-->得檢測年橋梁資料;)

[comment]: <> (跳過檢測沿用上次檢測資料-->得檢測年橋梁資料;)

[comment]: <> (得檢測年橋梁資料--進入下一檢測年---->生成訓練集與測試集資料;)
...
本章使用1992年至2019年之資料，訂定1999年為第一個檢測年，連續預測到2019年，共21次。
定義跳過檢測的次數上限為一次，預測模型與方案定義與上一章相同之設定，將各年預測結果之重大失誤率與資源節省率平均，比較三個方案下連續預測的表現。
預測結果記錄於下表：
...
1. 觀察不同方案的表現，可發現投入越多次檢測資料時，失誤率同樣呈現下滑趨勢，與前一章單年度預測的結果相同。
2. 此章的失誤率比起前一章都有所下降，可能原因為上一章所選用的2018年剛好是失誤率較高的一年，因此失誤率比起21年的平均表現高了一些。
3. 同樣比較資源節省率的部分，本章的節省率都比上一章低，應為在連續預測時設計了跳過檢測的次數上限，確保各橋梁檢測資料還是能隨著時間更新。

下圖為失誤率與資源節省率之逐年表現
![err_per_year](pic/err_per_year.png)
![ins_per_year](pic/ins_per_year.png)

各方案失誤率之逐年變化上差異不大。在節省率的部份，整體大致呈現「兩年高，兩年低」之變化，閥值降低後，各年之節省率也隨之減少。
節省率變動的趨勢主要與橋梁檢測頻率以及跳過次數上限的設定有關。FHWA規定多數橋梁皆為兩年接受一次檢測，因此可將所有橋約略分為奇數年檢測與偶數年檢測兩群。再加上此節中跳過次數上限設定為一次，使資料趨勢依照下圖之規律變動。
![ins_explanation](pic/ins_explanation.png)

[連續預測模型程式碼](code/ConsecutivePredictionModel.ipynb)
